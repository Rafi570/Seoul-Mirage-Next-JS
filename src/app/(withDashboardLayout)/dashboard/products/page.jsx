...
import { Plus, Eye, Trash2, ChevronLeft, ChevronRight, X, Loader2 } from 'lucide-react';
    ...
    useEffect(() => {
        const verifyAdmin = async () => {
            if (!authLoading) {
                if (user?.email) {
                    try {
                        const res = await axiosInstance.get(`/api/auth/role?email=${user.email}`);
            ...
            <div className="bg-white border border-gray-100 shadow-sm overflow-hidden">
                                ...
                                <tr key={p._id} className="border-b border-gray-50 hover:bg-[#FDFBF9] transition-colors">
                                    ...
                                    <td className="p-5 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => { setSelectedProduct(p); setIsDetailModalOpen(true); }} className="p-2 hover:bg-black hover:text-white transition-all rounded-full cursor-pointer"><Eye size={16} /></button>
                                            <button onClick={() => handleDelete(p._id)} className="p-2 hover:bg-red-500 hover:text-white transition-all rounded-full text-red-400 cursor-pointer"><Trash2 size={16} /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="p-6 flex justify-between items-center bg-white border-t border-gray-50">
                    ...
                    <div className="flex gap-2">
                        <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-2 border disabled:opacity-20 hover:bg-black hover:text-white transition-all cursor-pointer"><ChevronLeft size={18} /></button>
                        <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="p-2 border disabled:opacity-20 hover:bg-black hover:text-white transition-all cursor-pointer"><ChevronRight size={18} /></button>
                    </div>
                </div>
            </div>
            ...
            {isPostModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
                    <div className="bg-white w-full max-w-xl my-auto p-8 md:p-12 relative shadow-2xl animate-in zoom-in duration-300">
                        <button onClick={() => setIsPostModalOpen(false)} className="absolute top-6 right-6 text-gray-400 hover:text-black cursor-pointer"><X size={28} /></button>
                        ...
                        <form onSubmit={handleCreateProduct} className="space-y-6">
                            ...
                            <button type="submit" className="w-full bg-black text-white py-5 font-black uppercase tracking-[0.3em] text-[10px] hover:bg-[#222] transition-all cursor-pointer">Publish Now</button>
                        </form>
                    </div>
                </div>
            )}
            ...
            {isDetailModalOpen && selectedProduct && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
                    <div className="bg-white w-full max-w-5xl grid md:grid-cols-2 relative overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-500">
                        <button onClick={() => setIsDetailModalOpen(false)} className="absolute top-6 right-6 z-10 bg-white/50 p-2 rounded-full hover:bg-white cursor-pointer"><X size={24} /></button>
