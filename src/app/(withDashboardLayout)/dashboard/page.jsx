  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      setStats({
        ...
        avgOrderValue:
          orders.length > 0 ? (revenue / orders.length).toFixed(2) : 0,
        ...
        <div className="divide-y divide-gray-50">
          {stats.recentOrders.length > 0 ? (
            stats.recentOrders.map((order) => (
              <div
                key={order._id}
                className="p-5 flex items-center justify-between hover:bg-[#FDFBF9] transition-colors"
              >
                <div className="flex items-center gap-6">
                  <div className="bg-[#FEF4EC] p-3 border border-[#F5EEE6] rounded-sm">
                    {order.status === "Paid" ? (
                      ...
                      <Clock className="text-[#CCAF91]" size={18} />
                    )}
                  </div>
                  <div>
                    ...
                    <div className="flex gap-1 mt-2">
                      {order.items?.slice(0, 2).map((item, idx) => (
                        <span
                          key={idx}
                          className="text-[8px] bg-black text-white px-2 py-0.5 font-black uppercase tracking-tighter"
                        >
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`text-[8px] font-black uppercase px-3 py-1 rounded-full ${order.status === "Paid" ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700"}`}
                  >
            ...
            <div className="p-10 text-center text-gray-300 text-[10px] font-black uppercase tracking-widest">
